...
----
.
├── justfile                # Primary build automation (just command runner)
├── mustfile                # Policy enforcement script
├── deno.json               # Deno package manager configuration
├── rescript.json           # ReScript compiler configuration
├── README.adoc             # This document
│
├── bootstrap.res           # ReScript: Plugin entry point (-> bootstrap.js)
├── ZoteRhoTemplate.res     # ReScript: Core logic (-> MakeItRed.js)
├── RhodiumColorizer.res    # ReScript: Color utilities
│
├── config/                 # Nickel configuration sources
│   ├── manifest.ncl        # Generates manifest.json
│   ├── updates.ncl         # Generates updates.json
│   └── policy.ncl          # Hyperpolymath policy definition
│
├── src-2.0/                # Zotero 7+ plugin contents (packaged into XPI)
│   ├── manifest.json       # Generated from config/manifest.ncl
│   ├── style.css           # Plugin styling
│   └── locale/en-US/       # Fluent localization
│
└── build/                  # Build output (gitignored)
    └── zoterho-template-2.0.xpi
----
